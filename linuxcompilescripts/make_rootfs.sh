...
echo "OK"
#echo "qemu-debootstrap create file system done"
...
set -ex
apt-get -y update
apt-get -y install netbase net-tools ethtool udev iproute2 iputils-ping ifupdown isc-dhcp-client ssh ntp iw rfkill dialog locales adduser nano less wget dialog usbutils curl alsa-utils git mercurial make binutils  bzr bison libgmp3-dev screen gcc build-essential xfce4 xorg xfce4-goodies iceweasel 
apt-get clean
...
#cp install script 
#cp -ra quanzhi/skywire_install.sh  $DEST/usr/skywire_install.sh
#cp -ra quanzhi/skywirestart  $DEST/etc/init.d/skywirestart
#cp -ra quanzhi/rc.local  $DEST/etc/rc.local


#cp go and skywire
#cp -ra quanzhi/go $DEST/usr/local/go
#mkdir -p $DEST/root/go/{bin,pkg,src}
#mkdir -p $DEST/root/go/src/github.com/skycoin
#cp -ra quanzhi/skywire $DEST/root/go/src/github.com/skycoin/skywire
